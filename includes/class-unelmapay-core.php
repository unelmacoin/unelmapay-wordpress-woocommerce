...
class UNELMAPAY_Core
{
    ...
    public function sandbox_mode_field()
    {
        ...
        echo '<label><input type="checkbox" name="unelmapay_settings[sandbox_mode]" value="yes" ' . esc_attr($checked) . ' /> ';
    ...
    public function debug_mode_field()
    {
        ...
        echo '<label><input type="checkbox" name="unelmapay_settings[debug_mode]" value="yes" ' . esc_attr($checked) . ' /> ';
        ...
        <div class="wrap">
            ...
            ?>
                    ...
                    <p style="margin-top: 15px;">
                        ...
                        admin_url('admin.php?page=unelmapay-logs&action=clear'),
                        'unelmapay_logs_action',
                        'unelmapay_logs_nonce'
                        );
                        $download_url = wp_nonce_url(
                        admin_url('admin.php?page=unelmapay-logs&action=download'),
                        'unelmapay_logs_action',
                        'unelmapay_logs_nonce'
                        );
                        ?>
            ...
            if (isset($_GET['action'])) {
                if ($_GET['action'] === 'clear' && file_exists($log_file)) {
                    if (!isset($_GET['unelmapay_logs_nonce']) || !wp_verify_nonce($_GET['unelmapay_logs_nonce'], 'unelmapay_logs_action')) {
                ...
                if ($_GET['action'] === 'download' && file_exists($log_file)) {
                    ...
                    header('Content-Disposition: attachment; filename="unelmapay-logs-' . gmdate('Y-m-d-His') . '.txt"');
